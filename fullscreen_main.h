...
int _screen_x = 8;
int _screen_y = 8;
#define _screen_size _screen_x
#define KEY_FRESHLY_PRESSED 3

char keymap[256] = {0};
#ifdef NO_ESCAPE
void gcb_key_down(unsigned char key, int x, int y) { keymap[toupper(key)] = KEY_FRESHLY_PRESSED; }
#else
void gcb_key_down(unsigned char key, int x, int y) { keymap[toupper(key)] = KEY_FRESHLY_PRESSED; if (key == 27) exit(0); }
#endif
void gcb_key_up  (unsigned char key, int x, int y) { keymap[toupper(key)] = 0; } // XXX: toupper only works for letters, not numbers or punctuation. We should really do something more universal like keymap[shiftless(key)] and i would have to define the shiftless() function.

char special_keymap[256] = {0};
void gcb_special_key_down(int key, int x, int y) { special_keymap[key] = KEY_FRESHLY_PRESSED; }
void gcb_special_key_up  (int key, int x, int y) { special_keymap[key] = 0; }


#define keyboard_dz()     (!!keymap['W']-!!keymap['S']+!!special_keymap[GLUT_KEY_UP     ]-!!special_keymap[GLUT_KEY_DOWN     ]) // forward/backward
#define keyboard_dy()     (!!keymap['Q']-!!keymap['Z']+!!special_keymap[GLUT_KEY_PAGE_UP]-!!special_keymap[GLUT_KEY_PAGE_DOWN]) // jump/duck
...
#define PIXELS_PER_NDC_1 (_screen_size*0.5f*VIEWPORT_SCALE)
#else
#define PIXELS_PER_NDC_1 (_screen_size*0.5f)
#endif
...
void gcb_mouse_motion_with_pointer(int x, int y) {
 _mouse_dx = -_mouse_x;
 _mouse_dy = -_mouse_y;
 _mouse_x = (x - _screen_x/2) / PIXELS_PER_NDC_1;
 _mouse_y = (y - _screen_y/2) /-PIXELS_PER_NDC_1;
 _mouse_dx += _mouse_x;
 _mouse_dy += _mouse_y;
}
...
void gcb_mouse_motion_pointerless(int x, int y) {
 ...
 y -= center_y;
 if (x != 0 || y != 0) {
  _mouse_dx = x /-PIXELS_PER_NDC_1;
  _mouse_dy = y / PIXELS_PER_NDC_1;
...
void gcb_mouse_click(int button, int state, int x, int y) {
 switch (button) {
  case GLUT_LEFT_BUTTON:   _mouse_button_map[0] = (state==GLUT_DOWN);  break;
  case GLUT_MIDDLE_BUTTON: _mouse_button_map[1] = (state==GLUT_DOWN);  break;
  case GLUT_RIGHT_BUTTON:  _mouse_button_map[2] = (state==GLUT_DOWN);  break;
 }
 _mouse_x = (x - _screen_x/2) / PIXELS_PER_NDC_1;
 _mouse_y = (y - _screen_y/2) /-PIXELS_PER_NDC_1;
}
